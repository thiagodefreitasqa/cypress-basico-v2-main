...
describe('Central de Atendimento ao Cliente TAT', function() {
    ...
    it('2 - Preenche campos obrigatorios e envia formulario Sucesso',function(){
            ...
            cy.get('#email').should('be.visible').type(emailcorreto)
    ...
    it('3 - Exibe mensagem de erro ao Submeter o formulario com E-mail Incorreto',function(){
        ...
        cy.get('#email').should('be.visible').type(emailincorreto)
        cy.get('#open-text-area').should('be.visible').type(longtext,{delay:0})
        cy.get('button[type="submit"]').click()
    ...
    it('4 - Verifica se campo telefone continua vazio com valor não-numérico',function(){
        ...
        cy.get('#phone')
        .type('abcdefgghghg').should('have.text','')
    })
    it('4 - Exibe mensagem de erro quando o telefone se torna obrigatório mas não é preenchido antes do envio',function(){
        cy.get('#firstName').should('be.visible').type(nome)
        cy.get('#lastName').should('be.visible').type(sobrenome)
        cy.get('#email').should('be.visible').type(emailcorreto)
    ...
    it.only('5 - Preenche e limpa os campos nome, sobrenome, email e telefone',function(){
        cy.get('#firstName').should('be.visible').type(nome).should('have.value',nome).clear().should('have.value','')
        cy.get('#lastName').should('be.visible').type(sobrenome).should('have.value',sobrenome).clear().should('have.value','')
        cy.get('#email').should('be.visible').type(emailcorreto).should('have.value',emailcorreto).clear().should('have.value','')
        cy.get('#phone').type('113245558820').should('have.value','113245558820').clear().should('have.value','')
    })
